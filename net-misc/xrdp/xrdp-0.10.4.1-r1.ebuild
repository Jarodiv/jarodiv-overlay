...
src_configure() {
	...
	local myconf=(
		...
		# -- jpeg support --
		--enable-jpeg
		# The package supports explicit linking against libjpeg-turbo (no need for -ljpeg compat)
...
pkg_preinst() {
	# Either copy existing keys over to avoid CONFIG_PROTECT whining or generate new keys (but don't include them in binpkg!)
