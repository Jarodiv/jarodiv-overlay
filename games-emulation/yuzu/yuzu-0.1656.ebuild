...
MY_PV="mainline-${PV/./-}"
NX_TZDB_COMMIT="f6680093bca30265c161581fd813d4ddd33f1e3e"
NX_TZDB_VERSION="221202"

SRC_URI="https://github.com/yuzu-emu/yuzu-mainline/archive/${MY_PV}.tar.gz -> ${P}.tar.gz
  https://github.com/lat9nq/tzdb_to_nx/archive/${NX_TZDB_COMMIT}.zip -> ${PN}-nx_tzdb-${NX_TZDB_COMMIT}.zip"
...
PATCHES=(
	"${FILESDIR}/yuzu-skip-bundled-dependencies-1656.patch"
)
...
src_prepare() {
	...
	rm --recursive "${S}/externals/"{SDL,Vulkan-Headers,cpp-httplib,cpp-jwt,cubeb,dynarmic,enet,ffmpeg,libusb,mbedtls,opus,sirit,xbyak} || die

	mkdir -p "${S}_build/externals/nx_tzdb" || die
	cp "${DISTDIR}/${PN}-nx_tzdb-${NX_TZDB_COMMIT}.zip" "${S}_build/externals/nx_tzdb/${NX_TZDB_VERSION}.zip" || die
